function force = force(ij, n, q, deriv_u1, deriv_u2)
    ...
    %middle 
    if (i > 1 && i < n) && (j > 1 && j < n)
        topleftContribution     = deriv_u2(norm(q(:,i-1+((j-1)*n)) - q(:,i+j*n)));
        topContribution         = deriv_u1(norm(q(:,i+((j-1)*n)) - q(:,i+j*n)));
        toprightContribution    = deriv_u2(norm(q(:,i+1+((j-1)*n)) - q(:,i+j*n)));
        leftContribution        = deriv_u1(norm(q(:,i-1+j*n) - q(:,i+j*n)));
        rightContribution       = deriv_u1(norm(q(:,i+1+j*n) - q(:,i+j*n)));
        bottomleftContribution  = deriv_u2(norm(q(:,i-1+(j+1)*n) - q(:,i+j*n)));
        bottomContribution      = deriv_u1(norm(q(:,i+(j+1)*n) - q(:,i+j*n)));
